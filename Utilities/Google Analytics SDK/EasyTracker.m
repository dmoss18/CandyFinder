...
- (void)dispatchEvent:(NSString *)evnt action:(NSString *)actn label:(NSString *)lbl value:(NSInteger *)val {
    if (self.state == EasyTrackerStateNotTracking) {
        return;
    }
    NSError *error;
    if(![[GANTracker sharedTracker] trackEvent:evnt
                                    action:actn
                                     label:lbl
                                     value:val
                                     withError:&error]) {
...
- (void)applicationStateChanged:(NSNotification *)notification {
  ...
  if ([application applicationState] == UIApplicationStateActive) {
    if (self.state == EasyTrackerStateBackground) {
      ...
      } else if (![[GANTracker sharedTracker] trackEvent:@"Application"
                                                  action:@"background_to_foreground"
                                                   label:@""
                                                   value:0
                                               withError:&error]) {
    ...
    if (self.state == EasyTrackerStateForeground) {
      ...
      NSError *error = nil;
      if (![[GANTracker sharedTracker] trackEvent:@"Application"
                                           action:@"foreground_to_background"
                                            label:@""
                                            value:0
                                        withError:&error]) {
        NSLog(@"Error tracking foreground event: %@", error);
